...
TString inputDistr = "./InputRootFiles/NtuplePP5TeV_DATA_OS.root"; //
TString inputDistrMC = "./InputRootFiles/NtuplePP5TeV_MC_all.root"; // only prompt J/psi
...
Bool_t saveBkgParametersForPtIntegratedCase = kFALSE;
...
void  FitCDFLikelihoodPbPb(Int_t fitmode, Double_t ptMin, Double_t ptMax, Double_t bandLow, Double_t bandUp, TString resType, Double_t  centmin, Double_t centmax, Int_t mBkgOpt ){
 ...
 TString inputFileName = inputDistr; 
  ...
  if( !(ptMin==1.5 && ptMax==10.0) ){ 
      ...
      else SetStartingParameters(Form("inputFiles_1.5_10.0/bkgPtIntegrated/XBkgParameters_mass%1.1f_%1.1f_resTypeFF_FS_SS_pt1.5_10.0.root",bandLow,bandUp));
      // if(resType.Contains("FS")) SetStartingParameters(Form("inputFiles_3.0_5.0/XBkgParameters_mass%1.1f_%1.1f_resTypeFF_pt3.0_5.0.root",bandLow,bandUp));
        ...
        if(centmin==50 && centmax==90){
        TString massR;
        if(bandLow < 3.0) { massR = "L";
        if(bandLow == 2.60) massR.Append("1"); else massR.Append("2");  }
        if(bandLow > 3.0) { massR = "H";
        if(bandUp == 3.60) massR.Append("2"); else massR.Append("1");  }
  ...
  fithandler->SetMassWndHigh(massHigh);
   ...
   // likelihood fit for fixing x-bkg parameters (common to all candidate's types)
        ...
        if(ptMin > 0.) { paramInputValues[kWSym1L]=0.; paramInputValues[kLSym1]=0.; } //bkg Only 
        fithandler->SetParamStartValues(paramInputValues);
  ...
  if(fitmode == kFitXBkgMVAChi2){
   ...
   if(!(ptMin == 1.5 && ptMax == 10.0)){
     ...
     psProperBkgFit->FixParameter(8, 0.);
     psProperBkgFit->FixParameter(4, 0.);
  ...
  if(fitmode == kFitXBkgMVAChi2 || fitmode == kFitXBkgMVA) histpsproper->GetXaxis()->SetRangeUser(-3000.,3000.);   
  histpsproper->DrawCopy("E");
  TLatex *tex = 0x0;
