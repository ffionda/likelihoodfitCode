...
for(int ipt = 0; ipt<nptbins+1; ipt++){
    ...
    for(int imass = 0; imass < nmass; imass++) {
        ...
        printf("xx: %f, yy: %f \n",xx,yy);
        averageVal[ipt] += (1/(nmass*2))*(y1+y2);
        printf("%f \n",averageVal[ipt]);
    for(int jmass = 0; jmass < nmass; jmass++){
        uncertaintyVal[ipt] +=  (1/(nmass*2))*(1/(nmass*2))*gr[imass]->GetErrorY(ipt)*grquadratic[jmass]->GetErrorY(ipt)+(1/(nmass*2))*(1/(nmass*2))*grquadratic[imass]->GetErrorY(ipt)*grquadratic[jmass]->GetErrorY(ipt)+(1/(nmass*2))*(1/(nmass*2))*gr[imass]->GetErrorY(ipt)*gr[jmass]->GetErrorY(ipt);
        
}}}

TCanvas *c1 = new TCanvas("c1","A simple",200,10,700,500);
Double_t xVal[4] = {1.125,2.25,4.,7.5};
Double_t xValErr[4] = {1.125,1.125,1.,2.5};
c1->SetFillColor(42);
c1->SetGrid();
...
void plotAllResults(){
 for(int i=0; i<5; i++) plotResults(i,kFALSE);
 return;
}
