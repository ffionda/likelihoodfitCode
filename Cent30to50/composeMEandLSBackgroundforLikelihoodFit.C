void composeMEandLSBackgroundforLikelihoodFit(Double_t ptmin = 1.5, Double_t ptmax = 10., Double_t mMin = 2.5, Double_t mMax = 3.6, Double_t centMin = 30., Double_t centMax = 50., Int_t pairTypeMin = 1 , Int_t pairTypeMax = 2, Int_t polynOrd = 3, TString bkgName = "ME"){

TString inputFileName = Form("./MixedEventFiles/MixedEventResultsPt%.1f%.1fCent%.1f%.1fCand%.1d%.1d.root",ptmin, ptmax,centMin,centMax,pairTypeMin,pairTypeMax);
if(bkgName=="LS") inputFileName = Form("./LSFiles/LSResultsPt%.1f%.1fCent%.1f%.1fCand%.1d%.1d.root",ptmin, ptmax,centMin,centMax,pairTypeMin,pairTypeMax);
TFile *fmix = new TFile(inputFileName.Data(),"READ");
...
TH1 *mixScaledOrig;
if(bkgName == "ME"){
resBkg = (TH1*)mixScaled->Clone("residualBkg");
mixScaledOrig = (TH1*)mixScaled->Clone("mixEvOrig");
...
mixScaled->DrawCopy();
if(bkgName == "ME"){ resBkg->DrawCopy("same");
...
TFile *finvmassparamLikeME = new TFile(Form("inputFiles_%1.1f_%1.1f/InvariantMassBkg%s%s.root",ptmin,ptmax,bkgName.Data(),pairString.Data()),"RECREATE");
...
std::ofstream signalFracFile;
  ...
  signalFracFile.open (Form("inputFiles_%1.1f_%1.1f/FractionOfSignalMass%s%s.txt",ptmin,ptmax,bkgName.Data(),pairString.Data()), std::ofstream::out);
  signalFracFile << Form("%f \n",sigFrac);
  signalFracFile.close();

    if(bkgName == "ME") cMix->SaveAs(Form("MixedEventFiles/MEbackground_pt_%1.1f_%1.1f_cand_%s.png",ptmin,ptmax,pairString.Data()));
    else cMix->SaveAs(Form("LSFiles/LSbackground_pt_%1.1f_%1.1f_cand_%s.png",ptmin,ptmax,pairString.Data()));
return;
